...
class NextZmqExecutor:
    def __init__(self, error_utils, cmd_addr: Optional[str] = None, pub_addr: Optional[str] = None, interrupt_timeout: float = 0.5):
        ...
        self.interrupted_cell: Optional[int] = None  # Track which cell was interrupted
    ...
    async def execute_cell(self, cell_index: int, source_code: str, websocket: Optional[WebSocket] = None) -> Dict[str, Any]:
        ...
        try:
            self.req.setsockopt(zmq.RCVTIMEO, 100)  # Short timeout
            _ = self.req.recv_json()
            self.req.setsockopt(zmq.RCVTIMEO, -1)  # Reset to blocking
        except zmq.Again:
            ...
            try:
                self.req.setsockopt(zmq.RCVTIMEO, -1)
                self.req.close(0)
                self.req = self.ctx.socket(zmq.REQ)
                self.req.connect(self.cmd_addr)
            except Exception:
    ...
    async def interrupt_kernel(self, cell_index: Optional[int] = None):
        ...
        try:
            ...
            self.req.setsockopt(zmq.SNDTIMEO, 100)
            self.req.setsockopt(zmq.RCVTIMEO, 100)
            ...
            # Reset timeouts
            self.req.setsockopt(zmq.SNDTIMEO, -1)
            self.req.setsockopt(zmq.RCVTIMEO, -1)
    ...
    async def _force_kill_worker(self):
