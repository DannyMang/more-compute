...
import { Activity, Cpu, HardDrive, MemoryStick, Gauge, HardDriveDownload, ServerCrash, BarChart2 } from 'lucide-react';
      ...
      <div className="metrics-grid">
        {metrics?.cpu && (
          <Panel title="CPU Utilization" icon={<Cpu size={14} />}> 
            <BigValue value={fmtPct(metrics.cpu.percent)} subtitle={`${metrics.cpu.cores ?? '-'} cores`} />
        ...
        {metrics?.memory && (
          <Panel title="Memory In Use" icon={<MemoryStick size={14} />}> 
            <BigValue value={fmtPct(metrics.memory.percent)} subtitle={`${fmtBytes(metrics.memory.used)} / ${fmtBytes(metrics.memory.total)}`} />
        ...
        {metrics?.storage && (
          <Panel title="Disk Utilization" icon={<HardDrive size={14} />}> 
            <BigValue value={fmtPct(metrics.storage.percent)} subtitle={`${fmtBytes(metrics.storage.used)} / ${fmtBytes(metrics.storage.total)}`} />
        ...
        {hasGPU && (
          <Panel title="GPU Utilization" icon={<Gauge size={14} />}> 
            <BigValue value={fmtPct(metrics!.gpu![0].util_percent)} subtitle={`Temp ${metrics!.gpu![0].temperature_c ?? '-'}°C`} />
            <MiniChart data={history.map(h => (h.gpu && h.gpu[0]?.util_percent) || 0)} />
          </Panel>
        )}

        {metrics?.network && (
          <Panel title="Network" icon={<Activity size={14} />}> 
            <BigValue value={`${fmtBytes(metrics.network.bytes_recv)} ↓ / ${fmtBytes(metrics.network.bytes_sent)} ↑`} subtitle="total" />
          </Panel>
        )}

        {metrics?.process && (
          <Panel title="Process" icon={<ServerCrash size={14} />}> 
            <BigValue value={`${fmtBytes(metrics.process.rss)} RSS`} subtitle={`${metrics.process.threads ?? '-'} threads`} />
          </Panel>
        )}
      </div>
    </div>
  ...
  return (
    <div className="metric-panel">
      <div className="metric-panel-header">
        <span className="metric-panel-title">{icon}{icon && ' '}{title}</span>
      </div>
...
const BigValue: React.FC<{ value: string; subtitle?: string }> = ({ value, subtitle }) => (
